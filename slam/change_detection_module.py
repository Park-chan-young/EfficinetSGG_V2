...
def main(cfg : DictConfig):
    ...
    for frame_idx in trange(len(dataset)): # len(dataset) --> 1500
        ...
        if frame_changed:
            ...
            changed_classes_by_frame[frame_idx] = changed_classes
    ...
    out_text = f"changed_frames: {changed_frame_indices}"
    print("결과입니다 결과입니다 coarse change detection 결과입니다결과입니다 결과입니다 coarse change detection 결과입니다결과입니다 결과입니다 coarse change detection 결과입니다")
    print(out_text)
    print("결과입니다 결과입니다 coarse change detection 결과입니다결과입니다 결과입니다 coarse change detection 결과입니다결과입니다 결과입니다 coarse change detection 결과입니다")
    txt_lines = []
    for idx in changed_frame_indices:
        cls_list = changed_classes_by_frame.get(idx, [])
        txt_lines.append(f"{idx}: {', '.join(cls_list)}")

    save_path = exp_out_path / "coarse_cd_changed_classes.txt"
