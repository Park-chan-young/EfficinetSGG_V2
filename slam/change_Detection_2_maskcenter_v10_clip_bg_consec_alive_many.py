...
class ClipCenterCDCfg:
    ...
    tau_clip_sim: float = 0.80       # 후보로 인정할 최소 cosine sim
    ...
    # --- created ---
    tau_new_gaussian_dist: float = 0.50
    use_det_conf_gate_for_created: bool = True
    tau_conf_created: float = 0.6

    # --- matching for disappeared tracking ---
    tau_match_gaussian_dist: float = 0.50
    use_det_conf_gate_for_match: bool = False
    tau_conf_match: float = 0.0
...
def clip_center_cd_per_frame(
    ...
    for i in range(len(masks)):
        ...
        for j in cand:
            ...
            # best match (created 판정에 사용)
            if d < best_d:
                best_d = d
                best_obj = int(j)
                best_sim = sim_j

            # 1-to-many alive update (CLIP gate는 cand 구성 단계에서 이미 통과)
            if allow_match and (d <= float(cfg.tau_match_gaussian_dist)):
                matched_obj_indices.append(int(j))
                stat["matched_objects"].append({
                    "internal_obj_idx": int(j),
                    "object_index": int(before_cache.obj_ids[int(j)]),
                    "dist": float(d),
                    "sim": sim_j,
                })
...
def main(cfg : DictConfig):
    ...
    cfg_cd = ClipCenterCDCfg(
        clip_topk=5,
        tau_clip_sim=0.80,
        tau_new_gaussian_dist=0.50,
        use_det_conf_gate_for_created=True,
        tau_conf_created=0.6,
        tau_match_gaussian_dist=0.50,
