...
def main(cfg : DictConfig):
    ...
    alive_flags = np.zeros((len(before_objects),), dtype=np.uint8)

    # cache (invSigma 등) - 1회만
    cfg_scd = SceneCDCfg(
        tau_inlier_match=0.30,
        T_gate=9.0,
        use_label_gating=True,
        tau_conf=0.6,
        depth_scale=1.0,
        max_points_per_mask=None,  # 진짜 "전체" 쓰려면 None
    )

    gauss_cache, label_to_obj = _build_gaussian_cache(before_objects, eps=cfg_scd.eps)
    rng = np.random.default_rng(0) ####
    frame_to_unmatched: Dict[int, List[str]] = {}
    frame_stats: Dict[int, Dict[str, Any]] = {}

    rng_scd = np.random.default_rng(0)
    changed_classes_by_frame = defaultdict(list) ####
    ...
    for frame_idx in trange(len(dataset)): # len(dataset) --> 1500
        ...
        unmatched_labels, stat = scene_cd_per_frame_from_gobs(
            frame_idx=frame_idx,
            gobs=gobs,
            depth_array=depth_array,
            intrinsics_4x4=intrinsics.cpu().numpy(),  # 네 코드 그대로 :contentReference[oaicite:8]{index=8}
            pose_4x4=adjusted_pose,                   # adjusted_pose = dataset.poses[frame_idx] :contentReference[oaicite:9]{index=9}
        ...
        frame_to_unmatched[frame_idx] = unmatched_labels
        frame_stats[frame_idx] = stat
        # New_script_End_Coarse
    ...
    # 6) 프레임별 unmatched 클래스 라벨 저장
    out_txt_path = "frame_unmatched_labels.txt"
    with open(out_txt_path, "w") as f:
        for fi in sorted(frame_to_unmatched.keys()):
            f.write(f"{fi}: {frame_to_unmatched[fi]}\n")
    print(f"[INFO] saved per-frame unmatched labels -> {out_txt_path}")

    # 7) 사라진 객체 idx 저장 (alive_flags가 끝까지 0인 index)
    disappeared = np.where(alive_flags == 0)[0].tolist()
    out_dis_path = "disappeared_object_indices.txt"
    with open(out_dis_path, "w") as f:
        f.write(f"disappeared_object_indices: {disappeared}\n")
    print(f"[INFO] disappeared_object_indices: {disappeared}")
    print(f"[INFO] saved disappeared indices -> {out_dis_path}")
