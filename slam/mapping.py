...
def compute_text_similarities(
    ...
    """
    Compute the text similarities between detections and objects using CLIP text embeddings.
    ...
    """
    # (M, D), 각 detection이 갖고 있는 텍스트 feature
    det_text_fts = detection_list.get_stacked_values_torch('text_ft')  
    # (N, D), 각 object가 갖고 있는 텍스트 feature
    obj_text_fts = objects.get_stacked_values_torch('text_ft')  

    # 차원 확장해서 브로드캐스팅 가능하게 맞춤
    det_text_fts = det_text_fts.unsqueeze(-1)  # (M, D, 1)
    obj_text_fts = obj_text_fts.T.unsqueeze(0) # (1, D, N)

    # 코사인 유사도 계산 (M, N)
    text_sim = F.cosine_similarity(det_text_fts, obj_text_fts, dim=1)
...
#     text_sim: Optional[torch.Tensor] = None,   # NEW
#     use_text_sim: bool = False,                # NEW
#     text_alpha: float = 0.5                    # NEW: vis-text 혼합 비율
# ) -> torch.Tensor:
#     """
#     Aggregate similarities.
#     sims = (1+phys_bias)*spatial + (1-phys_bias)*(alpha*text + (1-alpha)*visual)
#     """
#     if match_method != "sim_sum":
#         raise ValueError(f"Unknown matching method: {match_method}")

#     if use_text_sim and text_sim is not None:
#         vis_text = text_alpha * text_sim + (1.0 - text_alpha) * visual_sim
#     else:
#         vis_text = visual_sim

#     sims = (1.0 + phys_bias) * spatial_sim + (1.0 - phys_bias) * vis_text
#     return sims
...
# 여기 수정 새롭게 설계한 것
def match_detections_to_objects_adaptive(   # NEW
    agg_sim: torch.Tensor,
    per_row_thresholds: torch.Tensor
) -> List[Optional[int]]:
    """
    Row-wise adaptive threshold matching.
    per_row_thresholds: (M,) each row's detection threshold.
    """
    M = agg_sim.shape[0]
    ...
    for i in range(M):
        row = agg_sim[i]
        mx = row.max()
        thr = per_row_thresholds[i].item()
        if mx <= thr:
            match_indices.append(None)
        else:
            match_indices.append(row.argmax().item())
    return match_indices
