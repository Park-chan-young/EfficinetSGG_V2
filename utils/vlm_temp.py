...
# def vlm_extract_object_captions(text: str):
#     """
#     Parse GPT output like:
#     [{'caption': 'a red bottle, material: plastic, weight: 300g, softness: semi-rigid'}, {...}]
#     """
#     text = text.replace('\n', ' ')
#     pattern = r'\[(.*?)\]'
#     match = re.search(pattern, text)
...
#         try:
#             result = ast.literal_eval(list_str)
#             if isinstance(result, list) and all(isinstance(obj, dict) and "caption" in obj for obj in result):
#                 return result
#         except Exception as e:
#             print(f"[Error] Failed to eval list: {e}")
#     print("No valid caption list found.")
#     return []
...
def get_obj_captions_from_image_gpt4v(client: OpenAI, image_path: str, label_list: list):
    ...
    user_query = (
        f"You are given an annotated image with object labels as follows: {label_list}. "
        "Each object is enclosed in a bounding box, and the number ID corresponds to a detected object in the image. "
        "Please describe each object in the image, and also estimate its material, weight (in grams), and softness."

        " To improve your estimation accuracy, follow these steps for each object:\n"
        "1. Estimate the relative size of the object from its bounding box.\n"
        "2. Predict the likely material based on its appearance and shape.\n"
        "3. Estimate the object's weight based on its size and material.\n"
        "4. Predict the softness based on the inferred material (choose from: soft, semi-rigid, rigid).\n\n"
        
        "Return a list where each item is a dictionary in this format:\n"
        "{'id': <id>, 'name': <name>, 'caption': '<brief description>, material: <material>, weight: <number>g, softness: <softness>'}\n\n"

        "Make sure to reason step-by-step internally before outputting the final result. "
        "Use your best judgment even if some information is uncertain."
    )
...
#     # 🧠 시스템 프롬프트: GPT가 역할을 이해하도록
#     system_prompt = (
#         "You are a visual perception expert. "
#         "You will be given an image and object labels with bounding boxes. "
#         "For each object, describe it briefly and append the estimated material, weight (in grams), and softness (soft, semi-rigid, or rigid). "
#         "Return the result as a Python list, where each item is a dictionary with a single 'caption' field. "
#         "Each caption should follow the format: "
#         "\"<object description>, material: <material>, weight: <number>g, softness: <softness>\""
#     )

#     # 📦 사용자 프롬프트: 라벨 정보 포함, 각 객체를 caption + 물성으로 정리해달라고 요청
#     user_query = (
#         f"Here is the list of object labels in the image (with ID): {label_list}. "
#         "Return a list of captions as described."
#     )
...
#         # 기존 캡션 외에 파싱 로직을 확장해야 함
#         # 예: vlm_answer_captions = parse_object_caption_and_properties(vlm_answer_str)
