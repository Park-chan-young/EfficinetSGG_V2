...
from conceptgraph.utils.vlm_temp import get_obj_captions_from_image_gpt4v, get_obj_rel_from_image_gpt4v, vlm_extract_object_captions
import cv2
import re
import numpy as np
import torch
...
def save_obj_json_temp(exp_suffix, exp_out_path, objects): #여기 수정
    """
    Save objects (with bbox summary) + parent/anchor/is_fixed to JSON.
    """

    def _to_py(v):
        # numpy → python scalar, lists/tuples/dicts 재귀 처리
    ...
    def _round_seq(seq, nd=2):
        if seq is None:
            return None
        return [ _round_float(x, nd) for x in seq ]

    def _serialize_parent(p, objects_ref):
        """
        p: {"rel","kind","idx","score","meta"}
        kind: "object"|"floor"
        """
        if not p:
            return None
        ...
        tag = name = None
        if kind == "object" and idx is not None and 0 <= int(idx) < len(objects_ref):
            idx = int(idx)
            tag = f"object_{idx+1}"
            name = objects_ref[idx].get("class_name", tag)
        elif kind == "floor":
            idx = None
            tag = "floor"
            name = "floor"

        # meta 안의 값들도 JSON-safe로 변환 + 소수 반올림(3자리)
        def _round_in_dict(d):
            out = {}
            for k, v in (d.items() if isinstance(d, dict) else []):
                if isinstance(v, (int,)):
                    out[k] = v
                elif isinstance(v, (float, np.floating)):
                    out[k] = _round_float(v, 3)
                elif isinstance(v, (list, tuple)):
                    out[k] = [_to_py(x) for x in v]
                elif isinstance(v, dict):
                    out[k] = _round_in_dict(v)
                else:
                    out[k] = _to_py(v)
            return out
        ...
        return {
            ...
            "meta": _round_in_dict(meta),
        }

    def _serialize_anchor(a, objects_ref):
        """
        a: {"idx","name","tag"} 또는 None
        동적 노드에만 존재할 수 있음.
        """
        if not a:
            return None
        ...
        if idx is not None:
            idx = int(idx)
            tag = tag or f"object_{idx+1}"
            if name is None and 0 <= idx < len(objects_ref):
                name = objects_ref[idx].get("class_name", tag)
        return {"idx": idx, "tag": tag, "name": name}
    ...
    for curr_idx, curr_obj in enumerate(objects):
        ...
        # bbox 요약 (없을 수 있으니 방어)
        bbox = curr_obj.get("bbox", None)
        if bbox is not None:
            try:
                bbox_extent = _round_seq(list(bbox.extent), 2)
            except Exception:
                bbox_extent = None
            try:
                bbox_center = _round_seq(list(bbox.center), 2)
            except Exception:
                bbox_center = None
        else:
            bbox_extent = None
        ...
        bbox_volume = None
        if bbox_extent and len(bbox_extent) == 3:
            try:
                bbox_volume = _round_float(bbox_extent[0] * bbox_extent[1] * bbox_extent[2], 2)
            except Exception:
                bbox_volume = None

        # parent / anchor / is_fixed
        is_fixed = bool(curr_obj.get("is_fixed", False))
        parent_ser = _serialize_parent(curr_obj.get("parent", None), objects)
        anchor_ser = _serialize_anchor(curr_obj.get("anchor", None), objects) if not is_fixed else None

        obj_dict = {
            "id": curr_obj.get("curr_obj_num", curr_idx + 1),
            "object_tag": curr_obj.get("class_name", ""),
            "object_caption": curr_obj.get("consolidated_caption", ""),
        ...
        # (옵션) 디버깅/후처리 편의용 필드 추가 저장 원하면 주석 해제
        # obj_dict["lower_y"] = _round_float(curr_obj.get("lower_y", None), 3)
...
# 여기 수정 visual feature 넘겨주는거
def save_clip_feats_npy(exp_suffix ,out_path, objects, key="clip_ft"):
    """
    objects의 obj[key]를 순서대로 쌓아 (N, D) npy로 저장.
    - 정규화/형변환/스케일 변경 없음 (원형 보존)
    """
    feats = []
    dtype = None

    for obj in objects:
        ft = obj[key]                     # torch.Tensor [D] or [1, D]
        if ft.ndim == 2 and ft.shape[0] == 1:
            ft = ft.squeeze(0)
        if dtype is None:
            dtype = ft.dtype              # 원래 dtype 보존 (예: torch.float32)
        assert ft.dtype == dtype, "모든 feature dtype이 동일해야 함"
        feats.append(ft.detach().cpu().numpy())  # 그대로 numpy로

    arr = np.stack(feats, axis=0)         # (N, D), dtype=원래 dtype과 동일(np.float32 등)
    vis_feature_out_path = Path(out_path)/ f"vis_feature_{exp_suffix}.npy"
    vis_feature_out_path.parent.mkdir(parents=True, exist_ok=True)
    np.save(vis_feature_out_path, arr)
    # out_path = Path(out_path)
    # out_path.parent.mkdir(parents=True, exist_ok=True)
    # np.save(out_path, arr)
    print(f"[OK] saved: {out_path} shape={arr.shape}, dtype={arr.dtype}")
    
# 여기 수정 text feature 추가하는걸로    
def save_text_feats_npy(exp_suffix ,out_path, objects, key="text_ft"):
    """
    objects의 obj[key]를 순서대로 쌓아 (N, D) npy로 저장.
    - 정규화/형변환/스케일 변경 없음 (원형 보존)
    """
    ...
    arr = np.stack(feats, axis=0)         # (N, D), dtype=원래 dtype과 동일(np.float32 등)
    vis_feature_out_path = Path(out_path)/ f"text_feature_{exp_suffix}.npy"
    vis_feature_out_path.parent.mkdir(parents=True, exist_ok=True)
    np.save(vis_feature_out_path, arr)
    # out_path = Path(out_path)
    # out_path.parent.mkdir(parents=True, exist_ok=True)
